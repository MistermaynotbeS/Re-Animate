...
if !INCLUDEDCHROMIUM!==n (
	if !CUSTOMBROWSER!==n (
		...
		if !DRYRUN!==n ( start http://localhost:4343/pages/html/main.html )
	) else (
		echo Opening Wrapper: Offline in your set browser...
		echo If this does not work, you may have set the path wrong.
		if !DRYRUN!==n ( start !CUSTOMBROWSER! http://localhost:4343/pages/html/main.html )
	)
) else (
	echo Opening Wrapper: Offline using included Chromium...
	pushd utilities\ungoogled-chromium
	if !APPCHROMIUM!==y (
		if !DRYRUN!==n ( start chrome.exe --user-data-dir=the_profile --app=http://localhost:4343/pages/html/main.html )
	) else (
		if !DRYRUN!==n ( start chrome.exe --user-data-dir=the_profile http://localhost:4343/pages/html/main.html )
	)
	popd
)
...
if !INCLUDEDCHROMIUM!==n (
	if !CUSTOMBROWSER!==n (
		echo Opening Wrapper: Offline in your default browser...
		start http://localhost:4343/pages/html/main.html
	) else (
		echo Opening Wrapper: Offline in your set browser...
		start !CUSTOMBROWSER! http://localhost:4343/pages/html/main.html >nul
	)
) else (
	echo Opening Wrapper: Offline using included Chromium...
	pushd utilities\ungoogled-chromium
	if !APPCHROMIUM!==y (
		start chrome.exe --user-data-dir=the_profile --app=http://localhost:4343/pages/html/main.html >nul
	) else (
		start chrome.exe --user-data-dir=the_profile http://localhost:4343/pages/html/main.html >nul
	)
	popd
)
goto wrapperidle
